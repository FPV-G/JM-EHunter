...
type PageTurnAnimationMode = 'page-flip' | 'rotate' | 'slide' | 'horizontal-slide' | 'none'
...
const defaultPageTurnAnimationMode: PageTurnAnimationMode = 'horizontal-slide'
...
function normalizePageTurnAnimationMode(value: any): PageTurnAnimationMode {
    ...
    // 向后兼容：将旧的 'realistic' 映射到新的 'page-flip'
    if (value === 'realistic') {
        return 'page-flip'
    }
...
export const store = reactive({
    ...
    flipDirection: 0, // 0: next, 1: pre (logical direction based on page index)
    physicalFlipDirection: 0, // 0: right-to-left motion, 1: left-to-right motion (physical animation direction)
    pageTurnAnimationMode: <PageTurnAnimationMode>defaultPageTurnAnimationMode,
    animationSpeed: 1.0, // animation speed multiplier (0.5 = 50% speed, 2.0 = 200% speed)
...
export const settingConf = {
    ...
    animationSpeed: {
        list: [0.5, 0.75, 1.0, 1.25, 1.5, 2.0],
        suffix: 'x'
    },
    autoFlipFrequency: {
...
export const storeAction = {
    ...
    setCurViewIndex: (val: number, updater: string) => {
        ...
        const getBookTurnDuration = (): number => {
            ...
            // Apply animation speed multiplier (inverse: higher speed = shorter duration)
        ...
        if (store.readingMode == 1 && store.pageTurnAnimationMode !== 'none') {
            // Allow animation override: immediately apply new page turn without waiting
            if (isBookTurning) {
    ...
    setThumbInfoDecodedSource: (index: number, src: string, ratio?: number) => {
        ...
        if (ratio && Number.isFinite(ratio) && ratio > 0) {
            ...
            store.thumbInfos[index].width = Math.max(1, Math.round(thumbnailHeight / ratio))
        }
    },
