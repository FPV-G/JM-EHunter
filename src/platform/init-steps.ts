...
export const C18_INITIALIZATION_STEPS: InitializationStepDefinition[] = [
...
export function createStepMap(stepDefinitions: InitializationStepDefinition[]): Record<string, InitializationStepDefinition> {
  return stepDefinitions.reduce<Record<string, InitializationStepDefinition>>((map, step) => { map[step.id] = step; return map }, {})
}

export function createStepUpdate(step: { id: string; label: string; order?: number }, status: InitializationStepStatus, detail?: string): InitializationStepUpdate {
  return { id: step.id, label: step.label, order: step.order, status, detail }
}

export function markCurrentPendingStepFailed(stepOrder: string[], stepStatus: Record<string, InitializationStepStatus>, stepMap: Record<string, { id: string; label: string; order?: number }>, reason: string, reportStep: (step: InitializationStepUpdate) => void): void {
